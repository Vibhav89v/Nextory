...
public class ChurnFreeTrialCampaignMember extends SuperTestScript {
	...
	public void churnFreeTrialCampaignMember() {
		...
		un = Excel.getCellValue(INPUT_PATH, "ChurnFreeTrialCampaign", 1, 0); // using
																				// the
																				// Free
																				// Trial
																				// or
		...
		if (memTypeCode.equalsIgnoreCase("203002") || memTypeCode.equalsIgnoreCase("202002")) {
			...
			if (subs.contains("FAMILJ") || subs.contains("GULD")) {
	...
	public void churnFreeTrialCampaignMemberNoSelection() {
		...
		if (memTypeCode.equalsIgnoreCase("203002") || memTypeCode.equalsIgnoreCase("202002")) {
			...
			if (subs.contains("FAMILJ") || subs.contains("GULD")) {
				...
				end.clickToUndo(); // clicking on the Angra button to go back to
									// account page

				acc.clickToEndSubscription(); // clicking on Avsluta abonnemang
												// button again to end the
												// subscription

				end.clickToEnd(); // clicking Avsluta button to confirm end
									// subscription.
			}
			...
			feed.clickToUndoButton(); // clicking Angra Button on the Feedback
										// Page, and Navigating back to Konto
										// Page

			acc.clickToEndSubscription(); // Starts to End the Subscription
											// again from Konto Page
			...
			feed.clickToEndButton(); // Clicking on End Button without Input
										// from Dropdown

			Alert a1 = driver.switchTo().alert();
			String alertText = a1.getText();
			if (alertText.equalsIgnoreCase("Välj ett alternativ")) // Validating
																	// for the
																	// Alert Box
																	// with the
																	// Suggestion
			...
			feed.clickFeedbackDropdown(); // Continuing with Churning Free
											// Trial/Campaign Member
			feed.selectNoTimeToUse();
			feed.clickToEndButton();
			feed.clickClearButton();
